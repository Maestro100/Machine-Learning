...
train_df = pd.read_csv(sys.argv[3], index_col="id", usecols=[0])
depths_df = pd.read_csv(sys.argv[4], index_col="id")
...
train_df["images"] = [np.array(load_img(sys.argv[1].format(idx), grayscale=True)) / 255 for idx in tqdm_notebook(train_df.index)]


# In[ ]:


train_df["masks"] = [np.array(load_img(sys.argv[2].format(idx), grayscale=True)) / 255 for idx in tqdm_notebook(train_df.index)]
...
x_test = np.array([(np.array(load_img(sys.argv[5].format(idx), grayscale = True))) / 255 for idx in tqdm_notebook(test_df.index)]).reshape(-1, img_size_target, img_size_target, 1)
